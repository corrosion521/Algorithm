...
def heapify(list,n,i):
    ...
    #왼쪽 애가 존재하고
    #왼쪽 애가 루트보다 더 클 때
    if left < n and list[root_largest] < list[left] :
        root_largest = left#swap 조건문에 트리거를 걸어주는 역할

    # 오른쪽 애가 존재하고
    # 오른쪽 애가 루트보다 더 클 때
    if right < n and list[root_largest]<list[right]:
        root_largest = right#이하 동문

    # 왼쪽, 오른쪽 자식들과 바꿔줘야 할 위치를 찾았을 때
    if root_largest !=i:
        # 찾아낸 값이랑 Swap
        list[i], list[root_largest] = list[root_largest],list[i]# i : 루트인덱스, root_largest : left or right
        #계속 heap의 형태를 갖출 때까지 실행, (!) 위치를 바꾸면 그 아래의 노드들과의 대소관계도 바뀔 수 있다는 것이기에 다시 heapify해줘야한다.
        heapify(list,n,root_largest)#root_largest : left or right 그 값.위에서 그 안의 원소는 바꿨으나(swap) 아직 root_largest,i자체의 값은 처음에 바뀐 그대로임.
...
def heapSort(list):
    ...
    #heap의 형태로 데이터를 저장하기
    for i in range(n-1,-1,-1): #C B T  중 가장 아래(마지막)의 노드부터 접근한다.
       heapify(list,n,i)#list,총길이,루트노드의 인덱스 전달

    # root에 있는 애랑, 마지막에 있는 애를 바꿔서 정렬해주고
    # 바뀐 애를 기준으로 다시 heapify를 실행합니다.
    for i in range(n-1,0,-1):# 0인덱스는 안하는 이유: 이 반복문은 바꿔주는 노드에 초점을 맞춤(마지막노드)
